...
# -*-coding:utf-8 -*-
#*******************************************************************************************
# **  文件名称：dm_user_moment_prefer_yyyymm.py
# **  功能描述：用户时段偏好模型月表
# **  输入表：  DM_USER_MOMENT_PREFER_BASE_YYYYMM    用户时段偏好基础信息月表
# **  输入表：  DW_USER_ALL_INFO_YYYYMM            用户中间层月表
# **  输出表:   dm_user_moment_prefer_yyyymm       用户时段偏好模型月表
# **
...
# ** ---------------------------------------------------------------------------------------
# **
# **  程序调用格式：python dm_user_moment_prefer_yyyymm.py 201312
# **
#********************************************************************************************
...
#Lib_Source               二级时段解析包路径
#===========================================================================================
#自定义变量声明---源表声明
#===========================================================================================
    source_dw_user_moment_prefer_base    = "DM_USER_MOMENT_PREFER_BASE_YYYYMM"
    source_dw_user_all_info              = "dw_user_all_info_yyyymm"

#===========================================================================================
#自定义变量声明---目标表声明
#===========================================================================================
    target_dm_user_app_prefer        = "dm_user_moment_prefer_yyyymm"

#===========================================================================================_
#自定义变量声明---临时表声明
#===========================================================================================
    temp_table_01                     ="dm_user_moment_prefer_yyyymm_tmp_01"
    temp_table_02                     ="dm_user_moment_prefer_yyyymm_tmp_02"
    temp_table_03                     ="dm_user_moment_prefer_yyyymm_tmp_03"
    temp_table_04                     ="dm_user_moment_prefer_yyyymm_tmp_04"
    temp_table_05                     ="dm_user_moment_prefer_yyyymm_tmp_05"
#===========================================================================
...
(
    ...
    ,prefer                DOUBLE     --偏好度
    ,rank_user_at_app      bigint     --用户偏好度的排名
    ,rank_app_at_user      int        --用户使用的二级时段安次数排倒序
...
from  %(source_dw_user_moment_prefer_base)s a
left outer join
       (select  moment_id
...
from   %(source_dw_user_moment_prefer_base)s m
left outer join %(temp_table_01)s n
 on m.moment_id=n.moment_id
...
select    '%(ARG_OPTIME_MONTH)s'
          ...
          ,m.GPRS_CNT
          ,m.prefer      --偏好度
          ,row_num_udf (moment_id) as rank_user_at_app
from (select * from %(temp_table_02)s distribute by  moment_id sort by moment_id,prefer desc) m
''' % vars())
...
select
       ...
       ,PHONE_NO             --手机号码
       ,moment_id               --时段标识
       ,GPRS_CNT_NEW GPRS_CNT       --本网用户浏览某类二级时段次数(中位数以下置0)
       ,prefer               --偏好度
       ,rank_user_at_app     --用户偏好度的排名
       ,ROW_NUMBER() OVER (PARTITION BY PHONE_NO ORDER BY  GPRS_CNT_NEW DESC ) AS rank_app_at_user --每个用户使用的二级时段按次数排倒序
       ,USER_CNT             --使用用户数
from (select  
             a.PHONE_NO
            ,a.moment_id
    ...
    left outer join
       (select
       moment_id         --时段标识
      ,count(moment_id) as USER_CNT       --使用用户数
 from %(temp_table_02)s
group by moment_id ) b
     on a.moment_id = b.moment_id  
        )  t
''' % vars())
...
#===========================================================================
# 第六步：利用H-Index算法对用户时段偏好进行打标签,并存放进表再次排序
#===========================================================================
    hivesql = []
    hivesql.append('''add jar /home/ocdc/app/hubei/lib/hdm-udf.jar''')
    hivesql.append('''create temporary function row_num_udf as "com.ailk.hdm.hive.udf.UDFRowNumber"''')
...
select
        ...
        ,PHONE_NO                   --手机号码
        ,moment_id                --时段标识
        ,GPRS_CNT                   --本网用户浏览某类二级时段次数
        ,prefer                     --偏好度
        ,row_num_udf (moment_id) rank_NEW  --用户偏好度的排名(打上偏好标签用户再次排名，用以打上偏好等级标签)
from (select *
      ...
      where rank_user_at_app < USER_CNT/2 --重度用户（排名在中位数之上）
        ...
        distribute by  moment_id sort by moment_id,prefer desc
        ) t
''' % vars())
...
left outer join
      (select
        ...
        group by moment_id) c
  on b.moment_id = c.moment_id
where a.%(Tas_MONTH_ID)s='%(ARG_OPTIME_MONTH)s'
''' % vars())
