...
# -*-coding:utf-8 -*-
#*******************************************************************************************
# **  文件名称：dm_user_app_type_prefer_yyyymm.py
# **  功能描述：用户应用偏好模型月表
# **  输入表：  DM_USER_app_PREFER_BASE_YYYYMM    用户应用偏好基础信息月表
# **  输入表：  DW_USER_ALL_INFO_YYYYMM           用户中间层月表
# **  输出表:   dm_user_app_type_prefer_yyyymm         用户应用偏好模型月表
# **
...
# ** ---------------------------------------------------------------------------------------
# **
# **  程序调用格式：python dm_user_app_type_prefer_yyyymm.py 201312
# **
#********************************************************************************************
...
#自定义变量声明---源表声明
#===========================================================================================
    source_dw_user_app_prefer_base    = "dm_user_app_prefer_base_yyyymm"
    source_dw_user_all_info           = "dw_user_all_info_yyyymm"

#===========================================================================================
#自定义变量声明---目标表声明
#===========================================================================================
    target_dm_user_app_prefer        = "dm_user_app_type_prefer_yyyymm"

#===========================================================================================_
#自定义变量声明---临时表声明
#===========================================================================================
    temp_table_01                     ="dm_user_app_type_prefer_yyyymm_tmp_01"
    temp_table_02                     ="dm_user_app_type_prefer_yyyymm_tmp_02"
    temp_table_03                     ="dm_user_app_type_prefer_yyyymm_tmp_03"
    temp_table_04                     ="dm_user_app_type_prefer_yyyymm_tmp_04"
    temp_table_05                     ="dm_user_app_type_prefer_yyyymm_tmp_05"
#===========================================================================
...
(
   ...
   ,APP_type_ID           STRING     --应用标识
   ,max_GPRS_CNT          DOUBLE     --用户浏览某类应用次数最大值
   ,max_FLOW              DOUBLE     --用户浏览某类应用流量最大值
   ,max_FREQ              DOUBLE     --用户浏览某类应用频率最大值
   ,min_GPRS_CNT          DOUBLE     --用户浏览某类应用次数最小值
   ,min_FLOW              DOUBLE     --用户浏览某类应用流量最小值
   ,min_FREQ              DOUBLE     --用户浏览某类应用频率最小值
...
(
    ...
    ,APP_type_ID                INT        --应用标识
    ,GPRS_CNT              INT        --本网用户浏览某类应用次数
    ,prefer                DOUBLE     --偏好度
)
row format delimited
fields terminated by '%(Tas_FS)s'
...
(
    ...
    ,prefer                DOUBLE     --偏好度
    ,rank_user_at_app      bigint     --用户偏好度的排名
    ,rank_app_at_user      int        --用户使用的应用安次数排倒序
...
from  %(source_dw_user_app_prefer_base)s a
left outer join
       (select  APP_type_ID
        ...
        from %(source_dw_user_app_prefer_base)s
      where %(Tas_MONTH_ID)s='%(ARG_OPTIME_MONTH)s'
        group by APP_type_ID
        ) b
    on a.APP_type_ID=b.APP_type_ID
...
from   %(source_dw_user_app_prefer_base)s m
left outer join %(temp_table_01)s n
 on m.APP_type_ID=n.APP_type_ID
...
select    '%(ARG_OPTIME_MONTH)s'
          ...
          ,m.GPRS_CNT
          ,m.prefer      --偏好度
          ,row_num_udf (APP_type_ID) as rank_user_at_app
from (select * from %(temp_table_02)s distribute by  APP_type_ID sort by APP_type_ID,prefer desc) m
''' % vars())
...
select
       ...
       ,PHONE_NO             --手机号码
       ,APP_type_ID               --应用标识
       ,GPRS_CNT_NEW GPRS_CNT       --本网用户浏览某类应用次数(中位数以下置0)
       ,prefer               --偏好度
       ,rank_user_at_app     --用户偏好度的排名
       ,ROW_NUMBER() OVER (PARTITION BY PHONE_NO ORDER BY  GPRS_CNT_NEW DESC ) AS rank_app_at_user --每个用户使用的应用按次数排倒序
       ,USER_CNT             --使用用户数
from (select  
             a.PHONE_NO
            ,a.APP_type_ID
    ...
    left outer join
       (select
       APP_type_ID         --应用标识
      ,count(APP_type_ID) as USER_CNT       --使用用户数
 from %(temp_table_02)s
group by APP_type_ID ) b
     on a.APP_type_ID = b.APP_type_ID  
        )  t
''' % vars())
...
select
        ...
        ,PHONE_NO                   --手机号码
        ,APP_type_ID                     --应用标识
        ,GPRS_CNT                   --本网用户浏览某类应用次数
        ,prefer                     --偏好度
        ,row_num_udf (APP_type_ID) rank_NEW  --用户偏好度的排名(打上偏好标签用户再次排名，用以打上偏好等级标签)
from (select *
      ...
      where rank_user_at_app < USER_CNT/2 --重度用户（排名在中位数之上）
        ...
        distribute by  APP_type_ID sort by APP_type_ID,prefer desc
        ) t
''' % vars())
...
left outer join
      (select
        ...
        group by APP_type_ID) c
  on b.APP_type_ID = c.APP_type_ID
where a.%(Tas_MONTH_ID)s='%(ARG_OPTIME_MONTH)s'
''' % vars())
