...
# -*-coding:utf-8 -*-
#*******************************************************************************************
# **  文件名称：dm_user_cont_type_prefer_yyyymm.py
# **  功能描述：用户内容偏好模型月表
# **  输入表：  DM_USER_cont_PREFER_BASE_YYYYMM     用户内容偏好基础信息月表
# **  输入表：  DW_USER_ALL_INFO_YYYYMM             用户中间层月表
# **  输出表:   dm_user_cont_type_prefer_yyyymm     用户内容偏好模型月表
# **
...
# ** ---------------------------------------------------------------------------------------
# **
# **  程序调用格式：python dm_user_cont_type_prefer_yyyymm.py 201312
# **
#********************************************************************************************
...
#自定义变量声明---目标表声明
#===========================================================================================
    target_dm_user_app_prefer        = "dm_user_cont_type_prefer_yyyymm"

#===========================================================================================_
#自定义变量声明---临时表声明
#===========================================================================================
    temp_table_01                     ="dm_user_cont_type_prefer_yyyymm_tmp_01"
    temp_table_02                     ="dm_user_cont_type_prefer_yyyymm_tmp_02"
    temp_table_03                     ="dm_user_cont_type_prefer_yyyymm_tmp_03"
    temp_table_04                     ="dm_user_cont_type_prefer_yyyymm_tmp_04"
    temp_table_05                     ="dm_user_cont_type_prefer_yyyymm_tmp_05"
#===========================================================================
...
(
    ...
    ,cont_type_id          INT        --四级内容标识
    ,GPRS_CNT              INT        --本网用户浏览某类四级内容次数
    ,prefer                DOUBLE     --偏好度
)
row format delimited
fields terminated by '%(Tas_FS)s'
...
(
    ...
    ,prefer                DOUBLE     --偏好度
    ,rank_user_at_app      bigint     --用户偏好度的排名
    ,rank_app_at_user      int        --用户使用的四级内容安次数排倒序
...
left outer join
    ...
    on a.cont_type_id=b.cont_type_id
where a.%(Tas_MONTH_ID)s='%(ARG_OPTIME_MONTH)s'
group by a.cont_type_id
''' % vars())
...
left outer join %(temp_table_01)s n
 on m.cont_type_id=n.cont_type_id
...
select    '%(ARG_OPTIME_MONTH)s'
          ...
          ,m.GPRS_CNT
          ,m.prefer      --偏好度
          ,row_num_udf (cont_type_id) as rank_user_at_app
from (select * from %(temp_table_02)s distribute by  cont_type_id sort by cont_type_id,prefer desc) m
''' % vars())
...
select
       ...
       ,PHONE_NO             --手机号码
       ,cont_type_id               --四级内容标识
       ,GPRS_CNT_NEW GPRS_CNT       --本网用户浏览某类四级内容次数(中位数以下置0)
       ,prefer               --偏好度
       ,rank_user_at_app     --用户偏好度的排名
       ,ROW_NUMBER() OVER (PARTITION BY PHONE_NO ORDER BY  GPRS_CNT_NEW DESC ) AS rank_app_at_user --每个用户使用的四级内容按次数排倒序
       ,USER_CNT             --使用用户数
from (select  
             a.PHONE_NO
            ,a.cont_type_id
    ...
    left outer join
       (select
       cont_type_id         --四级内容标识
      ,count(cont_type_id) as USER_CNT       --使用用户数
 from %(temp_table_02)s
group by cont_type_id ) b
     on a.cont_type_id = b.cont_type_id  
        )  t
''' % vars())
...
select
        ...
        ,PHONE_NO                   --手机号码
        ,cont_type_id               --四级内容标识
        ,GPRS_CNT                   --本网用户浏览某类四级内容次数
        ,prefer                     --偏好度
        ,row_num_udf (cont_type_id) rank_NEW  --用户偏好度的排名(打上偏好标签用户再次排名，用以打上偏好等级标签)
from (select *
      ...
      where rank_user_at_app < USER_CNT/2 --重度用户（排名在中位数之上）
        ...
        distribute by  cont_type_id sort by cont_type_id,prefer desc
        ) t
''' % vars())
...
left outer join
      (select
        ...
        group by cont_type_id) c
  on b.cont_type_id = c.cont_type_id
where a.%(Tas_MONTH_ID)s='%(ARG_OPTIME_MONTH)s'
''' % vars())
