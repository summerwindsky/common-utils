...
# ** ---------------------------------------------------------------------------------------
# **
# **  程序调用格式：python dm_user_app_prefer_yyyymm.py 201312
# **
#********************************************************************************************
...
#自定义变量声明---目标表声明
#===========================================================================================
    target_dm_user_app_prefer        = "dm_user_app_prefer_yyyymm"

#===========================================================================================_
#自定义变量声明---临时表声明
#===========================================================================================
    temp_table_01                     ="dm_user_app_prefer_yyyymm_tmp_01"
    temp_table_02                     ="dm_user_app_prefer_yyyymm_tmp_02"
    temp_table_03                     ="dm_user_app_prefer_yyyymm_tmp_03"
    temp_table_04                     ="dm_user_app_prefer_yyyymm_tmp_04"
    temp_table_05                     ="dm_user_app_prefer_yyyymm_tmp_05"
#===========================================================================
...
select    '%(ARG_OPTIME_MONTH)s'
          ...
          ,m.GPRS_CNT
          ,m.prefer      --偏好度
          ,row_num_udf (app_id) as rank_user_at_app
from (select * from %(temp_table_02)s distribute by  app_id sort by app_id,prefer desc) m
''' % vars())
...
from (select  
    ...
    left outer join
       (select
       APP_ID         --应用标识
      ,count(app_id) as USER_CNT       --使用用户数
...
select
        ...
        ,prefer                     --偏好度
        ,row_num_udf (app_id) rank_NEW  --用户偏好度的排名(打上偏好标签用户再次排名，用以打上偏好等级标签)
from (select *
      ...
      where rank_user_at_app < USER_CNT/2 --重度用户（排名在中位数之上）
        ...
        distribute by  app_id sort by app_id,prefer desc
        ) t
''' % vars())
