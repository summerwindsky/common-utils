...
select
   ...
   sum(case when a.dpd <> 0 and dpd <> 1 then a.LOAN_INIT_PRIN-a.PAID_PRINCIPAL else 0 end) as yq_sybj,  --逾期本金
   sum(case when a.dpd <> 0 and dpd <> 1 then a.LOAN_INIT_PRIN-a.PAID_PRINCIPAL else 0 end)/sum(case when a.LOAN_INIT_PRIN=a.PAID_PRINCIPAL then a.LOAN_INIT_PRIN else a.LOAN_INIT_PRIN-a.PAID_PRINCIPAL end) as yql
from(
select 
...
from
 ...
 where a.PLAN_ADD_DATE = '$last2day'
  group by a.PLAN_ADD_DATE;
" -B -o   /data1/share_dir/jyp/script/myc_overdue_receivedpay/data/$Yesterday/2day_overdue.txt 
